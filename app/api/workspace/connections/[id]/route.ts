...
async function loadCaseId(id: string): Promise<string | null> {
  const { rows } = await db.query('SELECT case_id FROM workspace_connections WHERE id = $1', [id])
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!caseId) return Response.json({ error: 'Connection not found' }, { status: 404 })
    if (!(await hasCaseAccess(caseId, session.id as string))) {
      return Response.json({ error: 'Access denied' }, { status: 403 })
    }

    const patch = await request.json()
    const query = buildUpdateQuery('workspace_connections', id, patch, { label: 'label' })
    if (!query) return Response.json({ error: 'No valid fields to update' }, { status: 400 })

    const { rows } = await db.query(query.text, query.values)
    return Response.json(rows[0])
  } catch (error) {
    console.error('Update workspace connection error:', error)
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await db.query('DELETE FROM workspace_connections WHERE id = $1', [id])
    return Response.json({ ok: true })
  } catch (error) {
    console.error('Delete workspace connection error:', error)
